...
export default function AuthForm({ setup = false, recovery = false, token = "", email = "" }: { setup?: boolean; recovery?: boolean; token?: string; email?: string }) {
  ...
  return <main className="vr-workspace flex min-h-screen items-center justify-center bg-[#0d0d0f] px-5 py-10 text-white">
    <div className="w-full max-w-md">
      ...
      <form onSubmit={submit} className="vr-surface rounded-2xl border border-[#29292d] bg-[#151517] p-7">
        ...
        <fieldset disabled={busy} className="space-y-4">
          ...
          <button className="vr-primary w-full rounded-lg bg-white py-3 font-medium text-black hover:bg-zinc-200 disabled:opacity-50">{busy ? "Aguarde..." : recovery ? "Salvar novo acesso" : setup ? "Criar meu acesso" : "Entrar"}</button>
        </fieldset>
