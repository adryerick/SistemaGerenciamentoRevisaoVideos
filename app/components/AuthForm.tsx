    ...
    <div className="w-full max-w-md">
      ...
      <form onSubmit={submit} className="rounded-2xl border border-[#29292d] bg-[#151517] p-7">
        ...
        <fieldset disabled={busy} className="space-y-4">
          ...
          <label className="block text-sm text-zinc-300">E-mail<input name="email" type="email" required defaultValue={email} maxLength={254} autoComplete="username" className={inputClass} /></label>
          <label className="block text-sm text-zinc-300">Senha<input name="password" type="password" required minLength={setup || recovery ? 12 : undefined} maxLength={128} autoComplete={setup || recovery ? "new-password" : "current-password"} className={inputClass} /></label>
          {(setup || recovery) && <>
