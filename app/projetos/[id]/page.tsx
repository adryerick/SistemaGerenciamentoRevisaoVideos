...
export default async function ProjectDetailsPage({
  ...
  const databaseProject = await prisma.project.findFirst({
  ...
  const project = toProjectDto(databaseProject);
  const videoVersions = databaseProject.videoVersions.map(toVideoVersionDto);
        ...
        <section className="min-w-0 flex-1 p-4 sm:p-8">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="rounded-xl border border-[#29292d] bg-[#151517] p-5">
              ...
              <strong className="mt-3 block text-2xl">{videoVersions.length ? `V${project.version}` : "Sem vídeo"}</strong>
            </div>
          ...
          <EditProjectPanel key={`${project.name}:${project.status}:${project.description}`} project={project} />
          <ProjectReviewPanels
            projectId={project.id}
